...
# Self-modification bridge (CRITICAL for self-modification)
try:
...
# Autonomous Engine (THE KEY TO AUTONOMY!)
try:
...
class JARVIS:
    ...
    def _init_self_mod(self):
        ...
        self.mod_bridge = None  # Bridge for AI→File operations
        self.autonomous_engine = None  # THE KEY TO AUTONOMY!
        ...
        # Initialize the bridge (CRITICAL for self-modification)
        if SELF_MOD_BRIDGE_AVAILABLE and SELF_MOD_AVAILABLE:
            try:
                self.mod_bridge = SelfModificationBridge(self)
                if self.debug:
                    print("[DEBUG] Self-modification bridge initialized - AI can now modify files!")
            except Exception as e:
                print(f"Warning: Could not initialize mod bridge: {e}")
        ...
        if AUTONOMOUS_AVAILABLE:
            try:
                ...
                print(f"Warning: Could not initialize autonomous engine: {e}")
    ...
    def _handle_command(self, command: str):
        ...
        if cmd_lower == 'help':
            # Use autonomous engine for help
            if self.autonomous_engine:
                result = self.autonomous_engine.process("help")
                print(result.output)
            else:
                self._show_help()
            return
        ...
        if cmd_lower == 'status':
            # Use autonomous engine for status
            if self.autonomous_engine:
                result = self.autonomous_engine.process("status")
                print(result.output)
                # Also show autonomous engine stats
                if hasattr(self, 'autonomous_engine') and self.autonomous_engine:
                    stats = self.autonomous_engine.get_stats()
                    print()
                    print(Colors.CYAN + "Autonomous Engine:" + Colors.RESET)
                    print(f"  Total Requests: {stats['total_requests']}")
                    print(f"  Direct Executions: {stats['direct_executions']}")
                    print(f"  AI-Assisted: {stats['ai_assisted']}")
                    print(f"  Chat Fallbacks: {stats['chat_fallbacks']}")
            else:
                self._show_status()
            return
        ...
        # ============================================
        if self.autonomous_engine:
            # Use autonomous engine FIRST
            result = self.autonomous_engine.process(command)
            
            if result.success:
                # Operation succeeded - show output
                print(result.formatted_output)
                
                # Show execution info if debug mode
                if self.debug:
                ...
                # If it was a chat intent and AI client exists, try pure chat
                if result.intent_type == "CHAT" and self.ai_client:
                    self._handle_ai_chat(command)
            
            return
        
        # ============================================
        # FALLBACK: Old bridge-based approach
        # ============================================
        if self.ai_client:
    ...
    def _handle_ai_command(self, command: str):
        ...
        try:
            ...
            if response.success:
                ...
                # Process any self-modification commands from AI response
                if self.mod_bridge:
                    ...
                    if mod_results['commands_found'] > 0:
                        ...
                        for exec_result in mod_results['executions']:
                            ...
                            if exec_result['success']:
                                ...
                                if exec_result.get('backup_id'):
                                    print(f"    Backup ID: {exec_result['backup_id']}")
                        ...
                        if mod_results['modified_files']:
                            ...
                            for f in mod_results['modified_files']:
                                print(f"    📄 {f}")
                            print()
                            print(Colors.GREEN + "  💾 Backups created automatically. Use 'backup list' to see them." + Colors.RESET)
    ...
    def _handle_ai_chat(self, command: str):
        """Handle pure AI chat (fallback for general questions)"""
        if not self.ai_client:
...
🔧 Self-Modification Commands:
  AI can now read, modify, create, and delete files!
  
  Just ask naturally:
    "read the main.py file"
    "add a debug command to main.py"
    "create a new utils.py with helper functions"
    "analyze the code_analyzer.py for issues"
    "list all files in core directory"

💾 Backup Commands:
  backup create       - Create backup of all files
  backup list         - List all backups
  backup restore <id> - Restore from backup

📁 File Operations (via AI):
  [READ:path/to/file]     - Read file
  [MODIFY:path/to/file]   - Modify file
  [CREATE:path/to/file]   - Create new file
  [DELETE:path/to/file]   - Delete file
  [ANALYZE:path/to/file]  - Analyze code
  [LIST:path/to/dir]      - List directory
  [SEARCH:pattern]        - Search codebase

⚡ Examples:
  "Read the main.py file and tell me what it does"
  "Add a new command called 'debug' to main.py"
  "Create a new file helpers.py with string utilities"
  "Fix the bug in the chat_storage.py file"
  "Optimize the performance of code_analyzer.py"
...
""")
    ...
    def _show_status(self):
        ...
        print(f"  Self-Mod Bridge:   {'✓' if self.mod_bridge else '✗'}")
        ...
        print()
        print("Self-Modification Status:")
        if self.mod_bridge:
            print(Colors.GREEN + "  🔧 AI can READ, MODIFY, CREATE, DELETE files!" + Colors.RESET)
            if self.backup_manager:
                print("  💾 Auto-backup: ENABLED")
            if self.code_analyzer:
                print("  📊 Code Analysis: AVAILABLE")
        else:
            print(Colors.YELLOW + "  ⚠ Bridge not initialized - AI cannot modify files" + Colors.RESET)
